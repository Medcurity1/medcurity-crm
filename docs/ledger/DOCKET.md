...
| B7 | Security Monitoring product (seed migration 20260803180000) | Summer (CRM request 2026-08-03) | On staging: product + $400 at 1-20 in all 12 books; larger tiers deliberately unpriced (manual path) until the bundle sheet is finalized — fill the ladder in a follow-up migration then. Summer books her 7/31 sale at $100 manually (pre-update price, per Makena). Promote on Nathan's word — she can't book the sale until it's on prod. | `git log origin/main --oneline \| grep -i "security monitoring"` = 0 hits (not yet promoted) | 2026-08-03 |
