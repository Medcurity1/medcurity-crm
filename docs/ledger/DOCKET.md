...
|---|---|---|---|---|
| B25 | Renewal cadence 1/2/3 years (tri-annual clients; Nathan's go 8/5) | BUILT + VERIFIED ON STAGING 8/5 (commit f880fcb, migrations 20260805200000/201000, CI 31057971287 green): accounts.renewal_cadence_years (canonical; every_other_year = synced legacy alias for 2, two-way BEFORE trigger, backfill converted North Sound to 2), all rule sites generalized to (N-1)-yr live window + (N-1)*12mo generator shift (derive/sweep/dashboard/generator/preview/audit/renewal_queue + Lost Customers/ARPC), form Renews Every select, detail layout row swapped. LIVE-VERIFIED: staging Unity Hospice set to 3 → badge former_client→client instantly, preview anniversary 2027-09-05 via contract_end_date_plus_24mo_cadence, config restored; form shows '3 Years'. AFTER PROMOTE: set Hope Health + Unity Hospice to 3 on prod (tell Rachel the select replaces the checkbox). | staging: accounts?name=ilike.*Unity*Hospice*&select=renewal_cadence_years shows 3; prod migration list lacks 20260805200000 | 2026-08-05 |
