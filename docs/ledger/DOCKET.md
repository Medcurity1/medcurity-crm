...
| A9 | Summer's auto-populate request (8/4): orig sales rep, FTE write-back, lead source from contact | Nathan (build go) | Summer Hume 2026-08-04 (low): (1) default original_sales_rep on new opps — field is filled on 1 of 2,199 opps, display-only when set, renewal generator copies parent OWNER not original rep (tweak to coalesce(parent.original_sales_rep_id, owner)); (2) FTE pull account→opp EXISTS (OpportunityForm.tsx:389-401, create-time on account pick, blank-only) but no write-back — 48% of accounts have FTE; add save-time write-back when account blank (helps close gate account_fte_range requirement); (3) lead_source required on opp create is JOE'S 7/10 directive (20260710186000, high priority — do NOT remove) + used by reports (NewCustomers, ARR dataset, builder); account-pull exists but only 2% of accounts have lead_source vs 63% of contacts — add primary-contact fallback + blank-only account write-back. Nothing pulls from contacts today (primary_contact_id stores the link only). Researched + recommendation delivered to Nathan 8/5; awaiting his go (triggers = migrations = Article II). | `grep -n "NEVER write back" src/features/opportunities/OpportunityForm.tsx` still present (no write-back built) + original_sales_rep_id fill count still ~1 on staging | 2026-08-05 |
