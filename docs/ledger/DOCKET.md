...
|---|---|---|---|---|
| C2 | Nexus "Your Day" overhaul | Nathan 2026-07-29. Ranked queue, landing flip, top-three cards, widgets, quick actions, and snooze are live. **8/18 PROD candidate:** the third Not today on the same item offers to stop that reminder or its category; Tune your list restores both. Product requests can be hidden as a category; regular tasks cannot. REMAINING: meeting-prep rows, Jordan M's plan review sign-off, and Home hard removal. Plan: `docs/nexus/your-day-plan.md`. | `rg -n "DayQueueHideDialog|DayQueueTuneList" src/features/nexus/Briefing.tsx` plus live third-dismissal and restore checks | 2026-08-18 |
...
| D19 | Submit Request dialog opens on the Collateral form by default — bugs get misfiled | Nathan 8/18 chose Product, CRM, Collateral and approved Production promotion. **STAGING VERIFIED:** the generic header button opens a three-choice screen in that exact order with no preselected form. Explicit deep links still open directly, Back preserves the dirty-input guard, Product puts Priority beside Title, Collateral keeps audience/format/Priority together, and both layouts stack on narrow screens. | `tests/requestChooser.test.ts`; Staging `fbbe14b`, CI 32214260923, desktop + 560px live checks | 2026-08-18 |
| D16 | No way to mark an account "still a customer" by hand | Nathan 8/18 approved the controlled override and Production promotion. **STAGING VERIFIED:** Account Status says whether it is automatic or manual, shows the saved reason, and gives admins/super-admins a reason-required Mark as Customer dialog. PostgreSQL enforces admin-only Customer overrides and protects them from non-admin replacement or clearing. | `tests/customerStatusOverride.test.ts`; Staging `4236019`, CI 32213030800, live admin check | 2026-08-18 |
...
| D12 | Deploy pipeline: pinned-old Supabase CLI + blanket edge-function redeploy | Nathan 8/18 approved the useful deployment fix and Production promotion. **STAGING VERIFIED:** both workflows use one tested change selector. Frontend/docs-only pushes deploy no functions; one function change deploys that function; `_shared` or an unsafe/unknown diff deploys every CI-managed function. JWT flags and manual-only exclusions are centralized and tested. The Supabase CLI remains exactly pinned at 2.47.2 because the selector does not require a version change; a separate upgrade can be tested later. | `tests/deployChangedFunctions.test.ts`; Staging CI 32213030800 + docs-only proof 32213299318 and 32214561384 | 2026-08-18 |
