...
| # | Item | Who | Detail | Verify | Checked |
|---|---|---|---|---|---|
| D1 | Campaigns public launch checklist | Nathan | (1) Jordan's copy into presets; (2) flip ACTIVE_ANNOUNCEMENT in AnnouncementBanner.tsx; (3) remove admin gates at the 3 flip points (App.tsx route, Sidebar adminItems, ContactsList right-click); (4) clamp leads_per_day (see D11). Also decide the 2 Playbook GH workflows disabled since 6/26 — likely superseded by the Campaigns engine. | `grep -n ACTIVE_ANNOUNCEMENT src/components/AnnouncementBanner.tsx` | 2026-07-27 |
...
| D11 | Clamp launch leads_per_day to inbox limit | (Campaigns Phase 5) | Default 25/day can exceed a sending inbox's limit (news@accessmedcurity.com is 15/day → "Room for ~0 more people/day"). Clamp at launch, or lower the default. | `grep -rn leads_per_day src/features/playbook/` | 2026-07-27 |
...
| F6 | Duplicate dashboard-metrics cache keys | TeamDashboard vs the standard report use colliding keys. | `grep -rn 'dashboard-metrics' src/features/` | 2026-07-27 |
...
| F12 | ImportsPen search has no debounce | Admin-only page. | `grep -rn debounce src/features/imports/` returns nothing | 2026-07-27 |
