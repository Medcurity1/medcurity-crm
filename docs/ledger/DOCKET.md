...
| B5 | Multi-tab idle-logout fix (Margaret's report) | Margaret via Nathan 2026-07-31 | useIdleLogout measured activity per tab but signed out globally — a forgotten background tab's 12h timer logged out the whole browser mid-work (warning invisible in a hidden tab; laptop sleep made it fire right after wake). Fix: shared last-activity stamp (sharedActivity.ts, localStorage) checked before warning AND before sign-out. **Margaret is on PROD — she keeps hitting it until this promotes.** | prod bundle's useIdleLogout has no sharedActivity import | 2026-07-31 |
