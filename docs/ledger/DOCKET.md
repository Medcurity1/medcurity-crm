...
- [QUEUED] 2026-07-17 · Nathan · Jordan-list mass lead→contact promotion prep (planning done, build pending Nathan's go): (a) optional tag-on-promote in Bulk Promote From File (apply a contact tag like "Jordan Clean Jul 2026" to every promoted contact, and ideally to skipped-duplicate keepers); (b) field-fill merge in archive_lead_as_duplicate (copy phone/title/etc. from dupe lead onto keeper contact where keeper is blank); (c) after the run, sweep skipped dupes via Admin → Data Cleanup lead-contact finder. Findings: bulk_promote_imports already skips exact-email dupes + previews counts; tombstone path carries activities + provenance but not field data; no batch marker on promoted contacts today (only original_lead_id).
...
- [STAGING] 2026-07-16 · Summer (via Requests form) · Batch of 5 CRM changes — ALL BUILT + VERIFIED on staging 2026-07-17 (commits f013c98..3db3959): (1) all 81 industry values in every picker via one shared source (closes the 2026-07-08 IndustryCategory gap; her actual gap was the lead-lists filter subset); (2) Contact Notes card just below the tiles (+ layout dedupe migration 20260717000002); (3) partner fields nested under the Partner checkbox, Partner Information section retired, lead source → Additional Information (migration 20260717000003; partner_account/partner_prospect UI-only removal, data intact); (4) archive (reason required) opened to sales/renewals for accounts+contacts (migration 20260717000004; delete/merge/restore stay admin); (5) clickable Active toggle in the account header w/ sub-status + follow-up + add-contacts-to-list flow (list membership auto-activates via Summer's Q8 trigger). Promote to prod with the next release.
