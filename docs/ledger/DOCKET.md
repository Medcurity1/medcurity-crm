...
|---|---|---|---|---|
| B8 | Requests → header popup (built 8/4 AM, polish round 8/4 PM after Nathan's test) | Ember-gradient Submit Request pill + popup (Requests sidebar tab gone). Nathan's 8/4 polish applied same day: humorous placeholders (billboard/beanbags/leaderboard), warm orangey-red palette for button + popup header (violet stays reserved for AI), Ask AI got an always-visible violet pill border to match, footer made opaque with the From line dropped, plus Nathan's one-time "Requests moved up here!" ember callout under the button (one-click dismiss or first use, localStorage `requests_moved_callout_dismissed_v1`, never returns). Live-verified on staging. Testing caution: product-BUG submits file real Jira tickets even from staging. Awaiting Nathan's re-look + prod go. | `grep -n 'from-orange-500' src/features/requests/RequestDialogProvider.tsx` + staging header shows the pill; prod header does NOT until promote | 2026-08-04 |
