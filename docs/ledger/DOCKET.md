...
| A13 | Summer's "active prospects" workflow (request 8/6) | Nathan (approve plan) → maybe Summer | She wants a low-friction way to track cold prospects (e.g. her Minnesota rural-health targets) without the Active toggle's required follow-up date, plus an auto-building list like her SQL smart list. Proposed plan (drafted 8/7, in chat for Nathan): (1) stop hard-requiring the follow-up date on activate — auto-fill a default (~30 days, editable) so one click marks an account Active+Prospecting; (2) a one-click "My Prospects" view = Active tab filtered to sub-status Prospecting + owner (the `prospecting` sub-status and `?sub=` filter already exist); (3) offer bulk import of her Minnesota list with owner+Prospecting pre-set. NOT BUILT — Nathan may send the plan to Summer first. | Ask Nathan; SalesStatusControl.tsx save() still hard-blocks activation without a date | 2026-08-07 |
