...
| A16 | Collateral: Azure app registration + secrets, then first Current file + hourly cron | Jordan Mayer / Nathan | The v1.1 sync (B36) is live but inert until the human Azure steps (spec §5): (1) register an app, grant Sites.Selected (Application) w/ admin consent, POST the READ role for it on the MedcurityInc site; (2) put GRAPH_TENANT_ID / GRAPH_CLIENT_ID / GRAPH_CLIENT_SECRET into the collateral-sync function secrets (driveId is baked, no secret needed); (3) Jordan promotes one file to Current and presses Sync — §6 checks run then (exactly one card, no Services Brochure, rename-safe links); (4) THEN enable the hourly cron for collateral-sync (not before — an unconfigured hourly job is noise per the 7/15 cron lessons). **HANDOFF DONE 8/13 (First Mate, verified in Nathan's Sent Items):** Nathan emailed Jordan on the "Collateral Library part 2" thread 8/12 5:28pm PT with azure-setup.md attached, told her the tab stays empty until the first Current file, and said "we can figure out the rest of the setup soon." Waiting on Nathan + Jordan + an M365 admin for the two consent clicks; not a fleet blocker. | Sync button on /collateral still returns configured:false; guide = docs/collateral/azure-setup.md | 2026-08-14 |
...
| # | Item | Who | Detail | Verify | Checked |
|---|---|---|---|---|---|
| B37 | Account page shows ALL activities (incl. ones logged on its contacts) | Molly Miller (request 8/13) | Her demo notes on a contact never appeared on the account's Activities tab. Root cause (repro'd on prod 8/14): 124 activities carry a contact_id but NULL account_id — 34 belong to contacts who ARE on an account (her 8/13 "Demo" w/ Donovan Roley is the newest) — because quick-create flows (global + Task, Log call, standalone attach picker) accept a contact without stamping its account. Fix, both sides: (1) read — account-scoped queries (timeline, Tasks panel, /activities?account_id=) now match `account_id = X OR contact_id in (account's contacts)` via accountScopeOrClause; (2) write — useCreateActivity stamps the contact's account_id when missing. No schema/data changes. | `grep -n accountScopeOrClause src/features/activities/api.ts` (helper + 2 uses) + `grep -n accountScopeOrClause src/features/activities/ActivitiesListPage.tsx` | 2026-08-14 |
| B38 | Amount column no longer truncates ("$28,...") in the opportunities list | Summer Hume (request 8/13) | InlineField's read view applied `truncate` to every field; numbers/dates now get `whitespace-nowrap` (auto-layout table widens the column to fit), free text (Next Step) keeps truncating. | `grep -n 'whitespace-nowrap' src/features/opportunities/OpportunitiesList.tsx` inside InlineField's read-mode span | 2026-08-14 |
...
| D18 | Detail-page Activity/Tasks side panel vanishes below 1280px window width | Jordan Scherich (request 8/14: "correspondence history for added users for an opportunity is gone", HRH) | DIAGNOSED 8/14, fix not yet authorized. Not data loss and not role-related: her HRH deal (0e8e323f, closed_won by her 8/14 12:21 PT) still has all 47 emails, RLS shows them to every role. The bug: DetailPageLayout's side_panel mode renders the Activity/Tasks aside as `hidden xl:block` with NO stacked fallback — at effective window width <1280px (small laptop, split screen, browser zoom) the whole correspondence panel is display:none on EVERY detail page (opps + accounts). Latent since 3bd5bcd (4/16); default pref is side_panel for everyone; the comment in the file falsely claims it "collapses to stacked". Compounding: the top Contacts/Products tabs are collapsed-by-default so the "added users" list also needs a click. Workarounds: widen/unzoom window, or Settings → Preferences → detail layout "stacked". Proper fix: render the stacked fallback below xl in side_panel mode. | `grep -n 'hidden xl:block' src/components/layout/DetailPageLayout.tsx` — still no below-xl fallback in the side_panel branch | 2026-08-14 |
