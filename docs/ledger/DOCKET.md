...
| A8 | Former Customer status wrong for every-other-year clients (Rachel's 8/5 request, MSD pending) | Nathan (prod go) + team (flag accounts) | Rachel Kunkel 2026-08-05: active clients showing "Former Customer". ROOT CAUSE: status derivation ignored accounts.every_other_year, so bi-annual clients flip to former_client in their planned gap year (North Sound 5e345e8d: SRA won Dec 2024, contract ended Dec 2025, next SRA proposal open Dec 2026). **BUILT + VERIFIED ON STAGING 8/5** (commit 6b2f5e0, migrations 20260805120000 + 20260805121000, CI run 31036542485 applied both): EOY-aware live window (+12mo past contract end, 730d close fallback) in derive + sweep + v_dashboard_metrics won_facts; NEW accounts trigger recomputes badge on flag flip (verified live: staging North Sound flipped former_client→client instantly); renewal generator + preview anniversary shifted +12mo for EOY (odd-cycle skip removed; verified preview shows anniversary 2026-12-09 via contract_end_date_plus_12mo_every_other_year, account_status client), renewal_queue + v_renewal_audit same cadence, gap-year churn suppressions cleaned (0 on staging); suppression view rebucketed North Sound contact as customer_account; Lost Customers (account-based) + ARPC reports EOY-aware client-side. REMAINING: (1) Nathan's prod promote go-ahead; (2) AFTER promote, EOY flags must be SET on prod bi-annual clients — prod North Sound has every_other_year=✗ (staging: 0 accounts flagged pre-test; I set staging North Sound's flag as the live test and left it); offer: candidate list from opp descriptions mentioning bi-annual/every-other + 2-year win cadence, Rachel/Summer confirm. (3) Deferred small gaps: rep_day_queue renewal nudge + KpiWidget/BuiltinReportWidget renewal tiles still window on raw contract_end (EOY renewals absent there; low value post-generator-fix). | Staging: accounts?id=eq.14821235-363b-455b-8181-e9045da3dfa9 shows every_other_year=true + customer_status=client; prod North Sound 5e345e8d badge still Former Customer until promote+flag | 2026-08-05 |
| A9 | Summer's auto-populate request (8/4): orig sales rep, FTE write-back, lead source from contact | Nathan (build go) | Summer Hume 2026-08-04 (low): (1) default original_sales_rep on new opps — field is filled on 1 of 2,199 opps, display-only when set, renewal generator copies parent OWNER not original rep (tweak to coalesce(parent.original_sales_rep_id, owner)); (2) FTE pull account→opp EXISTS (OpportunityForm.tsx:389-401, create-time on account pick, blank-only) but no write-back — 48% of accounts have FTE; add save-time write-back when account blank (helps close gate account_fte_range requirement); (3) lead_source required on opp create is JOE'S 7/10 directive (20260710186000, high priority — do NOT remove) + used by reports (NewCustomers, ARR dataset, builder); account-pull exists but only 2% of accounts have lead_source vs 63% of contacts — add primary-contact fallback + blank-only account write-back. Nothing pulls from contacts today (primary_contact_id stores the link only). **BUILT + VERIFIED ON STAGING 8/5** (Nathan's go; commits 4895690 + type-fix 8a68120, migrations 20260805160000 + 20260805170000, CI runs 31039224270/31039571488 green): BEFORE INSERT trigger opp_create_fill_defaults (rep=owner, lead source account→primary contact pair), AFTER trigger opp_backfill_account_basics (blank-only account FTE + lead source write-back, fte_count_to_range SQL mirror of employeesToFteRange), generator re-emit (child rep = coalesce(parent orig rep, owner); lead_source_detail now copied), form: rep defaults to owner + follows until touched, primary-contact lead source fill on pick. Verified live: REST throwaway (opp got rep+referral pair from contact; account got fte 25/21-50 + pair; propagate bounce filled opp fte_range; cleaned up) + UI form on North Sound (rep prefilled Nathan, picking Darren filled Lead Source=Other pre-save). Found+fixed same-day: 42804 enum/text coalesce (accounts.lead_source IS the enum — the A9 research note saying text was wrong). REMAINING: prod promote go-ahead. | staging: create opp via REST with null original_sales_rep_id → returns owner id; account write-back fills blank fte | 2026-08-05 |
