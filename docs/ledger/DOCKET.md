...
|---|---|---|---|---|
| B33 | Request listings tinted by form type (Nathan 8/12) | Nathan: as requests flood in, tell at a glance which is which — product ORANGE, CRM GREEN, collateral FADED GREY, as a Pulse brand-evolution moment. Shipped: shared REQUEST_TYPE_TINT map in requests/api.ts; every RequestCard row carries a 3px left accent bar + a color wash fading by mid-card + a type-colored hover ring (light+dark variants); mixed-list type chips and the detail-popup type badge went from outline to filled tint; the widget-builder Requests panel options carry matching legend dots. One shared card = widget, inbox popup, and Briefing deep-link all consistent. VERIFIED LIVE 8/12 (CI 31648276726 green on 3053350): widget shows the CRM row with emerald bar+chip and the collateral row slate, live in Nathan's session; orange product classes confirmed present in the deployed stylesheet (no pending product request existed to render, and none was created — staging shares the real Jira). | staging: Nexus Requests widget rows show colored left bars per type; `grep -n REQUEST_TYPE_TINT src/features/requests/api.ts` | 2026-08-12 |
| B32 | Finish Line dialog: fix blocked-close gaps in place (Molly's 8/12 request) | Molly's ticket: closing a deal that trips the close-readiness gate forced her to cancel her edits, hunt the account page, and retry from a fast-disappearing toast. Now ALL FOUR close surfaces (board drag, list inline stage, detail stage bar, edit form) open the Finish Line dialog when the gate blocks with fixable keys: every missing item (account phone / billing address / FTE range / contact email incl. quick-create first contact / assigned assessor) is an inline field, ZIP autofills state, live check-offs + progress track, one button saves everything to the right records, re-runs the REAL gate server-side, then completes the close (form surface re-submits itself with the assessor handed back; nothing in-flight is lost). Unfixable load failures still toast. New FinishLineDialog.tsx + fl-* design language in app.css (aurora header, checkered finish strip, gradient CTA, reduced-motion safe). Lib layer (missingKeys contract) was prepped by the 8/12 sister session; 482 tests green. VERIFIED LIVE on staging 8/12 (CI 31644965795 green on eb48638): detail-page path closed "ZZ Finish Line Demo Co"'s deal end-to-end (phone/address/FTE/email saved to the right records, ZIP 99201 auto-picked WA, Customer badge cascaded); form-CREATE path built "CO Training" straight to Closed Won incl. first-contact quick-create (Casey Checkers). Nathan's hands-on demo staged and left open: account "ZZ Finish Line TRY ME (Nathan's demo)", deal ce0d609e, contact Riley Ribbon (primary, no email). Demo rows are ZZ-prefixed disposables. | staging: block a close on an incomplete account from any surface → the Finish Line dialog opens instead of a toast; `grep -n missingKeys src/lib/closeReadiness.ts` | 2026-08-12 |
...
| G7 | Tasks organization pass | Molly (secondhand) via Nathan 8/12 | Molly's comment: tasks "not sorted". 8/12 read-only audit found the real gap: the Activities page (Type = Task) orders tasks by effective_at DESC (the logged date, same as calls/emails) while SHOWING a Due column — everywhere else (TasksPanel on account/contact/opp/lead + Nexus Today's Tasks) already uses the canonical due-then-priority comparator in taskOrder.ts. Recommendations delivered to Nathan 8/12: (a) task-filtered Activities adopts the canonical order or a sortable Due header, (b) urgency buckets (Overdue/Today/This week/Later/No date), (c) tinted due chips with relative phrasing, (d) surface the priority flag on TasksPanel rows (it sorts by it invisibly). Awaiting Nathan's pick. |
