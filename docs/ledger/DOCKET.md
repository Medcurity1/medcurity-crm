...
- [BLOCKED-on-Nathan] 2026-07-10 · Activate meddy-sweep + task-digest pg_cron on PROD: paste scripts/migration/prod-activate-meddy-taskdigest-cron.sql in the prod SQL editor (copies URL+key from the live email_sync job — no secret handling). ⚠ The 2026-07-11 15:00 UTC deadline PASSED un-pasted — today's task digest did NOT go out (task-digest has no trigger at all on prod; its GH schedule was removed). Paste ASAP. meddy-sweep still covered by its GH schedule meanwhile.
