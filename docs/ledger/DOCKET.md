...
|---|---|---|---|---|
| C3 | Requests → header popup relocation (IN PROGRESS — plan written, build awaiting Nathan's plan review) | Nathan 2026-08-04, HIGH priority — team already alerted and green-lit. Move Requests from a sidebar tab to a "Submit Request" header button (between search and Ask AI) opening a popup with the existing 3 forms (tightened, scrollable, refreshed look), deep-linkable to a starting tab (Nexus "Something missing?" → CRM tab; future Collateral page → Collateral tab; see A8). Must not downgrade — bug fast-path (MSD-957), attachments, all fields survive intact. Removes the Requests sidebar tab. Full plan: `docs/requests/relocation-plan.md`. | `ls docs/requests/relocation-plan.md` (plan exists) + `grep -n '"/requests"' src/components/layout/Sidebar.tsx` still present (not built) | 2026-08-04 |
...
| D13 | Calendar tab removal | Nathan | Confirmed 2026-08-04: no one uses it. Remove the sidebar tab + route + `ActivityCalendar`. Initial scope check (8/4) says LIGHT: one 439-line component (`src/features/activities/ActivityCalendar.tsx`), one route (App.tsx:181), one sidebar entry — zero deep links found (`grep -rln '/calendar' src/` hits only Sidebar). Nathan still wants the big agent sweep before build to catch non-obvious connections (event-creation flows, keyboard shortcuts, sectionName maps, tests). | `grep -n '"/calendar"' src/components/layout/Sidebar.tsx` still present | 2026-08-04 |
