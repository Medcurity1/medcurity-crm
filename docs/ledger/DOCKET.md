...
| A13 | Minnesota list bulk load onto PROD (Summer's prospects, part 3) | Nathan (explicit go — bulk data op) | Summer approved the plan 8/7 and sent MN_Independent_Hospitals_List.pdf (43 hospitals: 25 not in CRM, 10 in-CRM cold prospects, 5 warm, 2 existing customers, 1 open opp). Pending Nathan's go: create the ~25 missing accounts on prod (owner Summer, Active+Prospecting, follow-up +30d, city/state/website/phone/notes from the list) and flip the in-CRM cold ones to Prospecting where safe (no owner stealing — flag accounts owned by someone else). Claims in the PDF must be re-verified against prod before any write (dedup by name + website domain). Parts 1-2 (autofill + My Prospects view) built on staging 8/7 — see B27. | Prod: `accounts?billing_state=eq.MN` count unchanged since 8/7 = load not run | 2026-08-07 |
...
| # | Item | Who | Detail | Verify | Checked |
|---|---|---|---|---|---|
| B26 | Primary contact: one-click unselect from the account Contacts tab | Summer (request 8/7) | She could set a primary from the tab but clearing it required Edit → scroll → save. Fix: the green Primary badge is now a button — click it to clear (star swaps to an X on hover, toast confirms). Plain `is_primary=false` column update (same write path as the edit form checkbox), new `useUnsetPrimaryContact` in contacts/api.ts. No schema change. Verified live on staging 8/7 (set + unset both work). | `grep -n useUnsetPrimaryContact src/features/contacts/api.ts src/features/accounts/AccountContacts.tsx` | 2026-08-07 |
| B27 | Summer's prospecting flow: follow-up autofill + "My Prospects" view | Summer (request 8/6, plan approved 8/7) | Parts 1-2 of the plan: (1) flipping an account to sales-active with no follow-up date now pre-fills today+30 (editable) in BOTH the status popover and the edit form — shared `defaultFollowUpDate()` in lib/formatters.ts; the hard-block error stays as backstop only. (2) "My Prospects" toggle chip on the Accounts filter bar = sales-active + sub-status Prospecting + owner mine (all URL-backed, so it's bookmarkable). Part 3 (the MN data load on prod) is A13. Verified live on staging 8/7: date auto-fills on toggle, Prospecting save works, chip toggles both ways and showed exactly the one owned Prospecting account; test account reverted. | `grep -n defaultFollowUpDate src/lib/formatters.ts src/features/accounts/SalesStatusControl.tsx` + `grep -n myProspectsOn src/features/accounts/AccountsList.tsx` | 2026-08-07 |
