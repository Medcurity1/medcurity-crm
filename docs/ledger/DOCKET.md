...
| A12 | Set Hope Health + Unity Hospice to Renews Every 3 Years on prod | Claude (Chrome was down at promote time) or Rachel | The cadence build is LIVE ON PROD (promote 9aee0bf, 8/5 evening) but the two tri-annual accounts still sit at the default 1 year, so they still show Former Customer. 30-second fix per account: Edit → Contract & Renewal → Renews Every → 3 Years → Save (badge corrects instantly, staging-verified). Chrome MCP disconnected right at the follow-up step; do it next session or Rachel does it with her bi-annual flagging pass. | prod: Hope Health / Unity Hospice accounts show Renews Every = 1 (or badge still Former Customer) | 2026-08-05 |
