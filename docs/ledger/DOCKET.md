...
| A13 | Summer's "active prospects" workflow (request 8/6) | Nathan (approve plan) → maybe Summer | She wants a low-friction way to track cold prospects (e.g. her Minnesota rural-health targets) without the Active toggle's required follow-up date, plus an auto-building list like her SQL smart list. Proposed plan (drafted 8/7, in chat for Nathan): (1) auto-fill a default follow-up date on activate instead of requiring one; (2) add a "Prospecting" sub-status; (3) saved "My Prospects" view = Active tab filtered to Prospecting + owner. NOT BUILT — Nathan may send the plan to Summer first. | Ask Nathan; no code exists (`grep -rn 'prospecting' src/features/accounts/` empty) | 2026-08-07 |
