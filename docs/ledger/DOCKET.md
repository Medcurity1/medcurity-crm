...
| D17 | Close-gate toast: unambiguous wording + say where to fix | Jordan Scherich (8/10 bug ticket "Can't close a deal") | Gate logic is CORRECT — Extivita-RTP, LLC (prod acct 5c8bc50c) has phone NULL, so the toast said "still needed: Account phone number". Jordan read it as "account number", went looking for the Account Number field (removed from the form — auto-assigned since 20260424000003), retried for a week, filed a bug. Second confusion from this same gate (Molly 7/21 was FTE → fixed by deal back-fill). Cheap fix: relabel to "A phone number on the account record" + make formatCloseReadinessMessage name the account and where to edit ("open the account page → Edit"). Bigger option (Nathan's call): inline fill-the-gaps dialog at close time. | `grep -n '"Account phone number"' src/lib/closeReadiness.ts` still the ambiguous label | 2026-08-10 |
