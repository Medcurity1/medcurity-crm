...
|---|---|---|---|---|
| C5 | MSD-999 fast-path guardrails (Rachel via Makena 8/10) | Rachel's revised call, superseding the 8/4 review-gate reading of MSD-999 and RESOLVING A5 (filed-and-flagged stands): client-facing bugs KEEP the straight-to-dev fast path. Instead: (1) Helm classifier returns looksLikeBug; the Pulse form shows a bypassable warning when someone marks client-facing but the report reads as a request/enhancement (only time-sensitive client-facing BUGS may skip review); a bypass is stamped on the Jira ticket. (2) Auto-assign: straight-to-dev tickets → Makena (bug-intake + direct-file fallback), reviewer-approved filings → Rachel; env-overridable IDs. (3) Straight-to-dev email is importance:high. (4) That email links the ticket in Helm (/tickets?view=list&q=KEY) + Jira. Branch msd-999-fast-path-guardrails in BOTH repos, committed NOT pushed. Deploy: helm push→main auto-deploy; CRM staging→prod promote; note Rachel/Nathan have no Helm login yet (link hits login wall). Plan: docs/msd-999-client-facing-review-plan.md. | `git log origin/main --oneline | grep -i MSD-999` returns nothing (not merged) | 2026-08-10 |
