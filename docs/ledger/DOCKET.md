...
|---|---|---|---|---|
| B19 | Requests widget scrolls past the rows-shown cap | Nathan 8/4: the widget hard-capped at preview_count (default 5) with no way to reach request #6. Now every pending row renders inside a window sized to the rows-shown setting (~72px/row) with overflow scroll + a "scroll for the rest" footer hint; in-widget search filters the FULL list so matches beyond the fold surface too. | `grep -n "scroll for the rest" src/features/nexus/widgets/RequestsWidget.tsx` | 2026-08-04 |
...
| # | Item | Detail | Verify | Checked |
|---|---|---|---|---|
| C5 | Nexus ↔ Lists connection (Summer's day-one cluster) — BUILT on staging 8/4 | Nathan's distillation of her 4 requests + conversation: (1) NEW "List" WIDGET — any lead list as Nexus rows (static = members; smart = live rule minus exclusions), X-per-row to work the list (static: removes member; smart: writes a sticky exclusion so the rule can't re-add), rows-shown = scroll-window height, footer "Open list" deep-links to Reports→Lists (?list= param added). Her Warm Leads LIST is now widget-able; the tag-based Cold Call widget stays untouched (Molly may use tags). (2) SMART-LIST EXCLUSIONS (migration 20260805020000, lead_list_exclusions): subtracted by ALL resolvers incl. campaign audiences. (3) NEW RULE "New MQL or SQL within N months" in the smart-list editor (contacts.mql_date/sql_date window) — her auto-populating, work-through-able MQL/SQL list is now self-serve: smart list w/ that rule + owner=her, shown via the List widget. Widget type registered end-to-end (types, CHECKs, builder ListPanel, gallery, preview, tour catalog). Her richer display wishlist (last-touch days, notes, tasks inline) = docket idea for v2. ROUND 2 (Nathan 8/4 PM, all built): Lists tab REBUILT as two stages (card gallery → full-width workspace: filter + add-people side by side, 500-row table, right-click menu with Open contact / Add to another list / Start a campaign [admin] / Remove, admin Start-campaign-for-whole-list via QuickCampaignDialog, smart-rule chips in header); SMART LISTS ARE HYBRID now (hand-added members union the rule in ALL resolvers incl. campaigns; adding someone clears their exclusion) with a restorable 'removed and kept off' section; MQL/SQL rule timeframe is fully custom (number + day/week/month/year, stored as days, legacy months honored). | `grep -n lead_list_exclusions supabase/migrations/20260805020000_list_widget_and_smart_exclusions.sql` + List appears in the staging widget gallery | 2026-08-04 |
| C4 | Collateral tab (Jordan's V1 spec) — BUILT on staging 8/4, admin-only | Jordan's change-request doc (2026-08-04) implemented: /collateral page (AdminGate + Admin badge) ONLY — Nathan 8/4 PM: despite the doc's wording, Jordan wants NO contact/deal record presence, so those record tabs were built then REMOVED same day; the page is the single home. Jordan is drafting her enhancement round. Items 1-10: role flag = collateral_settings.visible_to_roles (RLS-enforced config, launch = admin/super_admin; sales flip = UPDATE + sidebar move); search across title+tags; chips FILTER never group (multi-tagged assets appear under every chip; OR within row, AND across, search ANDs); SRA family dropdown from the shared prefix (any future "Family — Variant" works free); Use chip defaults Send to Prospect; Copy Link on every card (+ per-user copy events, item 10); per-user default segments (Molly/Summer seeded per spec, "Save as my default" control); admin pinned row unaffected by filters; cards = title + type badge + product badges + amber Internal marker. Integration question ANSWERED: scheduled sync + manual entry (her option 2) — collateral-sync edge fn written against the real library drive (id captured from SharePoint), env-gated inert until the Graph app registration (Sites.Read.All) exists; admins add/edit/pin/archive manually meanwhile. Migration 20260805003000. 11 new logic tests. | `grep -n collateral_settings supabase/migrations/20260805003000_collateral_library.sql` + /collateral renders on staging | 2026-08-04 |
