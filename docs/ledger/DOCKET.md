...
| # | Item | Who | Detail | Verify | Checked |
|---|---|---|---|---|---|
| D1 | Campaigns public launch checklist | Nathan | (1) Jordan's copy into presets; (2) flip ACTIVE_ANNOUNCEMENT in AnnouncementBanner.tsx; (3) remove admin gates at the 3 flip points (App.tsx route, Sidebar adminItems, ContactsList right-click). Also decide the 2 Playbook GH workflows disabled since 6/26 — likely superseded by the Campaigns engine. ALSO (I15 prod check 2026-07-28, CORRECTED 2026-07-29 after a live status call): prod's SMARTLEAD_API_KEY IS set (playbook-smartlead status returns configured:true on prod). What prod lacks is only the NIGHTLY SWEEP SCHEDULE — no `app.campaigns_daily_sweep_url/key` (or `app.email_sync_url`) GUCs, so 20260722200000's cron never installed (verified: `cron.job` has no campaigns_daily_sweep row). Activation = set the two GUCs + re-run that migration, WITH Nathan's explicit go-ahead (it starts nightly engine activity against real Smartlead campaigns). (The leads_per_day clamp, formerly item 4 here, shipped 2026-07-27 — see SHIPPED.) | `grep -n ACTIVE_ANNOUNCEMENT src/components/AnnouncementBanner.tsx` | 2026-07-27 |
