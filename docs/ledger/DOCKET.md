...
|---|---|---|---|---|
| B36 | Collateral v1.1 (Jordan Mayer's 8/11 spec via Nathan 8/12) | Full rebuild of the tab to her spec, scope-locked to /collateral (§0 — diff touches ONLY collateral files). §1 data-source fix: sync reads ONLY the Sales Collateral drive (driveId baked as default, drive-scoped endpoint, no site search; source-guard test pins it + bans the Shared Documents driveId), Status=Current only, verbatim column mapping incl. new stage/status/last_reviewed/owner_name (migration 20260812220000), links keyed on drive item id. §3 read-only: Add/Edit/Archive UI + hooks + dialog DELETED, manual rows purged, RLS tightened to admin UPDATE-only (pin toggle is the sole client write; prefs/copy-events unchanged); sync deletes rows that leave the library (mirror, not archive). §4 redesign: Medcurity-brand light canvas fully scoped to .collat-* (Plus Jakarta Sans route-injected, peach hero = the one warm moment, ice chips → navy selected, SRA parent chip w/ caret, white 14px cards + file-type glyph in ice square, teal primary = Request collateral, blue link actions, amber Review-due >180d + Internal badges, teal Send-to-Prospect badge, navy top-edge pinned row, branded empty state). Defaults on load: Use=Send to Prospect + saved segments. 6 new tests (isReviewDue, fileKind, sync source guard), 503 green. AWAITING HUMAN STEPS (§5, Jordan/Nathan): Azure app registration (Sites.Selected, READ role on MedcurityInc site) + secrets GRAPH_TENANT_ID/CLIENT_ID/CLIENT_SECRET into the Supabase function, then promote a file to Current + Sync (§6 checks), then enable hourly cron. ⌘K deviation flagged to Jordan: global ⌘K stays Pulse's global search per her own §0 defer-to-Pulse rule. ROUND 2 (Nathan 8/12 same day): tab is now THEME-AWARE (dark navy palette under .dark, scoped vars only, light mode unchanged = Jordan's reference); ALL em-dashes removed from collateral copy and code (SRA family-value parsing kept, it reads SharePoint data); (a bottom request-callout strip was added then REMOVED same day: Nathan realized the hero's teal Request collateral button already covers it); step-by-step Azure guide written at docs/collateral/azure-setup.md (registration, secret, Sites.Selected consent, Graph Explorer site-read grant, Supabase secrets, Jordan's proof test). | staging: /collateral follows the app theme (dark canvas in dark mode) + callout strip renders; `grep -n SALES_COLLATERAL_DRIVE_ID supabase/functions/collateral-sync/index.ts` | 2026-08-12 |
