...
| B10 | Nexus Metrics strip (Home's clickable Key Metrics carried over) | Nathan 8/4: metrics were Home's most-used feature because they're at the top and clickable. Built as its own section under the three surfaced briefing items, above Your widgets: compact 6-across clickable tiles (same kpi-registry queries, deep links, and accents as Home; SMALLER per his ask), reusing the user's existing Home metric picks (same localStorage config + Home's KpiConfigDialog for choosing), plus a Nexus-only Show/Hide toggle in Customize (default SHOWN, localStorage per user). UPDATE 8/4 PM: Nathan reviewed and called for the Metrics WIDGET's removal — done same day (see B14's commit): widget code deleted (~1,800 lines incl. registry/panel/tests), gallery entry gone, migration 20260804200000 deletes all placed instances (rows backed up to RLS-locked backup tables first) and narrows the widget_type CHECKs. He loves the strip. | `grep -n MetricsStrip src/features/nexus/NexusPage.tsx` wired | 2026-08-04 |
...
| B14 | Nexus drag-and-drop rework (Nathan 8/4: "glitchy and odd") | Root cause: rectSortingStrategy assumes a uniform grid, but Nexus is two independent variable-height stacks, so mid-drag previews jumped/overlapped. New model in NexusGrid + FeaturedWidgets + SortableWidget: cards hold still, a DragOverlay clone follows the pointer (shadow + tilt), the origin card fades to a ghost slot, the hovered card highlights as the drop target, drop settles with a 200ms animation; droppable rects re-measure continuously. | `grep -n STATIC_WHILE_DRAGGING src/features/nexus/NexusGrid.tsx` | 2026-08-04 |
| B15 | Nexus LAUNCH: the swap, staged 8/4 (Nathan: promote at lunch after his test) | Built as one batch: "/" now lands on Nexus (Home moved to /home + bottom of the sidebar group, kept until confirmed unneeded); Nexus is the top tab with an emerald "Launched" badge; NEXUS_IS_LANDING flipped true (arms the first-visit tour + the Home-layout carry-over import); announcement banner active (nexus-launch-2026-08); tour grown 3→6 learning steps (briefing, metrics strip [skips if hidden], widgets, the 9-widget-type catalog list, customize, Submit Request button); migration 20260804220000 seeds Recent Wins (the high-five widget) for every initialized user with grid room + adds it to the default layout; metrics strip already defaults SHOWN for everyone. G-then-H + old "/" links land on Nexus. Round 2 (8/4 PM, Nathan's review): Nexus badge is now a blue gradient, Home carries a red Leaving soon badge; high-five confirmed intact on the wins widget (it shows on wins closed within 7 days; staging just had none since early July — trigger proven live by re-firing the Brayden Test $421 deal, button now visible for his test). | `grep -n "NEXUS_IS_LANDING: boolean = true" src/features/nexus/landing-flip.ts` + staging "/" redirects to /nexus | 2026-08-04 |
...
| H2 | Nexus dashboard default tab | RESOLVED-pending-promote 8/4: Nathan called the swap (B15); Nexus is the default on staging. Row retires when B15 ships to prod. |
