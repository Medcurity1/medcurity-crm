...
|---|---|---|---|---|
| C3 | Requests → header popup relocation (STAGING — built 2026-08-04, awaiting Nathan's staging test then prod go) | Nathan 2026-08-04 (plan approved same day, "good to go to build on staging" + creative-freedom note on the look). BUILT: aurora-gradient popup (RequestDialog + provider + header SubmitRequestButton between search and Ask AI), all 3 forms extracted intact (RequestForms.tsx — bug fast-path, attachments, validation untouched), dirty-close/switch discard guard (new safety upgrade), sticky submit footer, /requests route = redirect that opens the popup honoring ?tab=, Nexus "Something missing?" opens popup on CRM tab, briefing "Open request" now points at the Admin requests inbox, sidebar tab REMOVED, RequestsPage.tsx + dead RequestWidgets.tsx deleted. Plan: `docs/requests/relocation-plan.md`. NOTE for testing: product-bug submits file real Jira tickets even from staging. | `grep -n SubmitRequestButton src/components/layout/AppLayout.tsx` (wired) + `grep -c requests src/components/layout/Sidebar.tsx` = tab gone; on prod the header button is absent until promote | 2026-08-04 |
...
| D13 | Calendar tab removal — SWEEP DONE, ready to build on Nathan's go | Nathan | Confirmed 2026-08-04: no one uses it. Full agent sweep ran 8/4 (Nathan's ask): removal = DELETE ActivityCalendar.tsx (439 lines, all symbols file-local) + EDIT App.tsx (lazy import :56, route :181), Sidebar.tsx (nav entry + now-unused CalendarIcon lucide import), AppLayout pathMap `calendar:` entry; plus 5 doc files. Zero deep links, zero tests, zero dead code left behind, no calendar-only data flows (everything visible in /activities). KEEP list confirmed: outlook-calendar-sync edge fn (separate feature), renewals_calendar builtin report, WidgetShell `calendar` icon key (persisted in user widgets), ~11 files' date-icon imports, Google Calendar coming-soon connector card. One UX note: calendar was the only view merging task due-dates + activities on one time axis — pure UX loss, no data loss. Post-removal verify greps must exclude `.claude/worktrees/` (4 stale repo copies) and `dist/`. | `grep -n '"/calendar"' src/components/layout/Sidebar.tsx` still present | 2026-08-04 |
| D14 | Nexus tab launch checklist | Nathan | 2026-08-04: after Nathan's Nexus testing wraps — move tab toward the top of the sidebar, add/verify walkthrough (NexusTour.tsx already exists), flip ACTIVE_ANNOUNCEMENT banner, give the tab a highlight badge ("New" or "Launched" — Nathan leaning something like Launched since it isn't new), then start the Home-tab retirement (the 3rd tab removal, with C3's Requests and D13's Calendar). Gated on his testing + H2 (Home stays default until his flip call). | `grep -n ACTIVE_ANNOUNCEMENT src/components/AnnouncementBanner.tsx` still null + Nexus entry carries no badge in Sidebar.tsx | 2026-08-04 |
