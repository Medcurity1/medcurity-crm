...
| A17 | Idle-improvement survey: 10 candidate topics await Nathan's picks | Nathan 8/17 ("find 10 different topics... I'll pick a few") | Five read-only code sweeps run 8/17; 10 topics T1-T10 + full file evidence in `docs/audit/2026-08-17-idle-improvement-survey.md`. Includes 3 real bugs found in passing (merge-undo bigint::uuid abort, Mailchimp ingest 42P10 on every new campaign, dashboard sums truncated at 1000 rows) — those live inside T4/T9/T1. When Nathan picks, mark picks in the audit doc and open one D-row per picked topic; unpicked topics stay in the doc as the idea bank. | Audit doc's "PICKED:" line still says "(none yet)" | 2026-08-17 |
