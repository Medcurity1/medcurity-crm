...
| # | Item | Detail | Verify | Checked |
|---|---|---|---|---|
| F16 | "Widget Rows do not adjust for warm leads" (Summer 8/4) | Repro unclear: ColdCallListWidget passes widget.preview_count as limit correctly; her report may concern the warm-first section inside ColdCallBody or a Custom Report widget on the warm-lead tag. Investigate with her exact widget config; may be MOOT once she switches to the new List widget (C5). | Ask Summer which widget, or check ColdCallBody's warm-section row math | 2026-08-04 |
