...
| # | Item | Who | Detail | Verify | Checked |
|---|---|---|---|---|---|
| B31 | Nexus "Open task" card deep-links the task (was the activities log) | Molly Miller (8/12 ticket "Open Task blue bar") | primaryAction's task/campaign_task branch hardcoded `/activities` (the daily email/call log) while every other card kind deep-linked its record. Now: `taskIdOf(row)` (row.task_id first, item_key 'task:<id>' fallback) → `/activities/<id>` — the task's own page with the account link + manage controls, exactly her ask. 6 new tests incl. request-row regression guard. Same staging batch: account-header action row wraps instead of running off-screen (Nathan 8/12, Merge button was the tipping point) + merge wizard footer buttons get their gap back (`sm:gap-0` removed). | `grep -n "activities/" src/features/nexus/Briefing.tsx` shows the deep link; awaiting promote = `git log origin/main --oneline \| grep -i "open task"` empty | 2026-08-12 |
| B30 | Guarded account pair-merge for non-admin users | Nathan 8/11 ("adapt the existing account merge tool so a normal user can safely merge duplicates") | Migration 20260812000000: shared `_merge_accounts_core` (EXECUTE revoked; both wrappers reuse it) + `merge_account_pair` RPC gated by has_crm_write_role() (sales/renewals/admin/super_admin; read_only + NULL rejected), exactly 2 live accounts, row locks in id order, REQUIRED updated_at concurrency snapshots, server whitelist of 38 choosable columns (unknown key = loud reject), compliance flags OR'd never choosable, single transaction, audit row carries survivor_before + field_choices + merged_via. Core now also moves 4 tables the June tool stranded (account_attachments, campaign_enrollments, deal_wins, partner_contract_summaries) — admin tool inherits the fix. (First pass also targeted contact_account_links; the very first staging test merge caught that the table was dropped back in May — transaction aborted whole, nothing changed, fix migration 20260812000001 removed it.) undo_account_merge (still admin-only) restores the new tables AND the survivor's profile fields for pair-tool merges. UI: Merge button on AccountDetail (canWrite gate, next to Archive), 3-step wizard (pick via AccountCombobox → side-by-side review of every field incl. blanks, survivor recommendation = won-deal-then-older, per-row swap, conflict highlighting → plain-language confirm w/ admin-undo note). 17 new logic tests (defaults/blank/identical/conflict/swap/null-clear/whitelist/exclusions/survivor rule). E2E VERIFIED on staging 8/12 with disposable ZZ Merge Test accounts (archived after): wizard walked end to end incl. success toast; RPC applied choices (description copied, ZIP explicitly cleared) + moved contact/activity + archived loser w/ reason + full audit row; anon caller rejected ("requires a CRM write role"); stale updated_at token rejected with the friendly reopen message; admin undo returned survivor_fields_restored=true and put BOTH accounts back byte-identical. RESIDUAL: no live sales-role JWT existed to prove the ALLOW side for non-admins on staging (gate is has_crm_write_role(), the same helper every rep write already relies on); worth a 30-second check by any sales user post-promote. PARTNER LINKS FULLY REVERSIBLE (8/12, migration 20260812000002 + 9-test executable spec): core snapshots substitution originals, undo reverses substitutions then resurrects deleted links (skip-and-count only for hard-deleted endpoints/existing identical pairs; anything else aborts whole); June-era merges gain deleted-link restoration retroactively; both admin copies updated. Verified live with a 4-link fixture covering every path (kept/collision/self-link/substitution): merge via ORDINARY CLICKS ONLY through all 3 wizard steps + toast, admin undo via Data Cleanup UI, all 4 links restored exactly with original ids, zero duplicates; fixture archived. | `grep -n "merge_account_pair" supabase/migrations/20260812000000_guarded_pair_merge.sql` + `grep -n account_partners_substituted supabase/migrations/20260812000002_partner_links_restorable_undo.sql`; awaiting promote = `git log origin/main --oneline \| grep -i "pair-merge"` empty | 2026-08-12 |
