...
2. **Website form (D1) — ✅ BUILT 2026-07-20, REVISED same day per Nathan in person: NO task** (ownership differs every time; the team already gets pinged externally). As built: visible contact (import_status null), company on import_company AND in notes, extra fields (industry/website/employees/revenue/description) preserved in notes, Website tag, lead_source mapping unchanged, email dedup across all 3 contact email columns (fail-open — a dedup error never blocks the create); a submission that matches a PENDING pen row self-promotes it out of the pen (hand-raisers are never buried); bells only, to users with the My Settings → Notifications "website inquiry" toggle ON (opt-in; migration 20260720130000 seeds Nathan/Summer/Molly ON; 1-hour double-submit guard); response keeps lead_id AND the legacy lead object alongside contact. Deployed to STAGING via a staging-only CI-list exception; ⚠️ PROD stays manual — Nathan runs `supabase functions deploy inbound-lead --no-verify-jwt` at cutover (the website points at prod, so prod's fn keeps writing leads until then — by design, the pen handles stragglers). (S)
3. **+ 4. Pen cutover — ✅ SHIPPED TO STAGING + LIVE-VERIFIED 2026-07-20** (migrations 20260720100000/120000, commits 7c4c1cd/732f28f; 4-row walkthrough green: pen import, create-vs-attach promote, avoid w/ DNC, archive; legacy strip + one-click sweep in place, sweep click itself is Nathan's). Remaining in-piece: SalesforceImport still OFFERS a 'leads' entity writing the frozen table (remove in piece 5/9); report-dataset pen-hiding deferred to piece 5 (engine lacks a base-filter hook). Original scope for reference: SF/CSV importer's `leads` entity → writes pen-flagged contacts; Imports tab repoints to pen-flagged contacts; bulk actions ported (promote → clear flag + account attach, reusing the battle-tested `bulk_promote_imports` account-matching logic; archive → contact archive + reason; avoid → contact `do_not_contact`/`do_not_market` + archive reason); filters port; lists per D2. Prod has ZERO pending leads, so the cutover has no data to migrate there — **clean/archive staging's leftover lead rows first as the rehearsal**. (M-L, the one half-day-ish piece)
