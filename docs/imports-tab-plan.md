...
2. **Website form (D1) — ✅ BUILT 2026-07-20** (Nathan's answer: BOTH task + notification; baseline = lead behavior, as a contact). As built: visible contact (import_status null), company kept on import_company (no auto-created accounts from a public form), extra fields (industry/website/employees/revenue/description) preserved in notes, Website tag, lead_source mapping unchanged, email dedup across all 3 contact email columns; every submission (incl. repeats) → ONE follow-up task (owner = oldest active admin, deterministic — swap to a specific owner is a one-liner) + bell notification to all active admins, deduped per person per day via activities(source='website_form', external_id); response shape unchanged (lead_id alias kept). Deployed to STAGING via a staging-only CI-list exception; ⚠️ PROD stays manual — Nathan runs `supabase functions deploy inbound-lead --no-verify-jwt` at cutover (the website points at prod, so prod's fn keeps writing leads until then — by design, the pen handles stragglers). (S)
