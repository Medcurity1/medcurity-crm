-- =====================================================================
-- COPY THE WHOLE THING. Paste into Supabase Dashboard → SQL Editor →
-- New query → click Run.
--
-- Creates picklist_options table if missing + seeds every picklist
-- with canonical values. ~30 seconds. Idempotent: re-runnable.
-- =====================================================================

-- Step 1: table + RLS + indexes
...
-- Step 2: seed canonical values
-- Opportunity Contract Length (stored as months on backend; user sees 1/3 year)
...
-- Account Industry (deduped SF list)
insert into public.picklist_options (field_key, value, label, sort_order) values
...
-- Contact Business Relationship Tag
insert into public.picklist_options (field_key, value, label, sort_order) values
...
-- Contact Lead Source (mirrors opp lead_source)
...
-- Lead Credential / Time Zone / Relationship Tag (mirror contact)
insert into public.picklist_options (field_key, value, label, sort_order)
select 'leads.credential', value, label, sort_order
from public.picklist_options where field_key = 'contacts.credential'
on conflict (field_key, value) do nothing;

insert into public.picklist_options (field_key, value, label, sort_order)
select 'leads.time_zone', value, label, sort_order
from public.picklist_options where field_key = 'contacts.time_zone'
on conflict (field_key, value) do nothing;

insert into public.picklist_options (field_key, value, label, sort_order)
select 'leads.business_relationship_tag', value, label, sort_order
from public.picklist_options where field_key = 'contacts.business_relationship_tag'
on conflict (field_key, value) do nothing;

-- Verify (returns counts per field)
select field_key, count(*) as values_count
from public.picklist_options
group by field_key
order by field_key;
