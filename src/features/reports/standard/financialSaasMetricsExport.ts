...
const DEFINITIONS: [string, string | null, string | null][] = [
  ...
  ["Churn % ($)",
    "Renewal revenue lost / (renewed + lost) for the period. The share of the renewal cohort that churned. Always 0-100% (you cannot lose more than what was up for renewal). New business is excluded; only closed_lost (existing-customer non-renewals) count as lost.",
    "f_financial_saas_metrics_quarterly.churn_pct_dollars / ttm_churn_pct_dollars"],
  ["Churn % (#)",
    "Customers lost / (renewed + lost) for the period (logo churn). Same renewal-cohort basis as Churn % ($), counted by distinct account. Always 0-100%.",
    "f_financial_saas_metrics_quarterly.churn_pct_customers / ttm_churn_pct_customers"],
  ["Gross Retention Rate",
    "Share of the renewal cohort retained; always ≤ 100%.",
