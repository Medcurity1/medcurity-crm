...
const DEFINITIONS: [string, string | null, string | null][] = [
  ...
  ["Churn % ($)",
    "Revenue lost / (active revenue + lost) for the period — losses as a share of the whole client book. Lose the entire book = 100%; lose nothing = 0%. New business is never in the churn math; only closed_lost (existing-customer non-renewals) count as lost.",
    "f_financial_saas_metrics_quarterly.churn_pct_dollars / ttm_churn_pct_dollars"],
  ["Churn % (#)",
    "Customers lost / (active customers + lost) for the period (logo churn). Same whole-book basis as Churn % ($), counted by distinct account. Always 0-100%.",
    "f_financial_saas_metrics_quarterly.churn_pct_customers / ttm_churn_pct_customers"],
  ["Gross Retention Rate",
    "Share of the client book retained; always ≤ 100%.",
