...
function drawHeader(doc: import("jspdf").jsPDF, windowLabel: string, generatedAt: Date) {
  ...
  doc.text(`Generated ${genStr} · PulsePoint CRM`, PAGE_W - MARGIN, 19, { align: "right" });
}
...
function drawChart(doc: import("jspdf").jsPDF, quarters: QuarterMetrics[]) {
  ...
  for (let i = 0; i <= 4; i++) {
    ...
    doc.text(`${(100 * i) / 4}%`, plotX + plotW + 2, y + 1);
  }
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (churnPct > 100) {
      doc.setFillColor(255, 255, 255);
      doc.rect(x, plotY + 1.2, barW, 0.8, "F");
      doc.setFillColor(...RED);
      doc.triangle(
        xCenter(i) - 1.4, plotY + 4.4,
        xCenter(i) + 1.4, plotY + 4.4,
        xCenter(i), plotY + 1.6,
        "F",
      );
