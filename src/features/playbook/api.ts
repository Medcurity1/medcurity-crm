...
// Client-side timeout removed: invokeAI is shared across mutation and
// query actions (interpret creates DB records, resolve creates provenance,
// link/discard mutate state). Aborting a mutation that succeeds server-side
// but gets aborted client-side creates inconsistency. The server's own
// 90s callClaude timeout is the authoritative deadline for AI calls.
