...
import { Rocket, Flame, Wand2, Sparkles, Clock, Layers, ArrowRight } from "lucide-react";
...
import { useCampaignTemplates } from "./api";
    ...
    <div className="space-y-2">
      <div>
        ...
        <p className="text-xs text-muted-foreground">
          Pick a proven sequence, see every step, then launch it on a list or contact. Email steps send automatically; calls and LinkedIn become your tasks in Up Next.
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          ...
          {/* Start from scratch */}
          <Card className="py-0 cursor-not-allowed opacity-70 border-dashed overflow-hidden" title="Building a custom sequence from scratch lands next.">
            ...
            <>
              ...
              <DialogFooter className="flex-col sm:flex-row gap-2 sm:items-center sm:justify-between">
                <p className="text-[11px] text-muted-foreground order-2 sm:order-1">
                  Launching + editing from a template is the next build.
                </p>
                <Button variant="ai" disabled className="order-1 sm:order-2">
