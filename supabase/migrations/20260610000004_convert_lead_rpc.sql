-- ---------------------------------------------------------------------
-- Atomic, role-safe lead conversion.
--
-- Fixes two issues with the previous client-side conversion:
--   A) It failed for NON-admin users: the final step archives the lead,
--      and archiving is admin-only (20260415000004), so sales/renewals
--      users hit "row violates RLS for table leads".
--   B) It was NOT atomic: account + contact were inserted before the
--      lead update; if the update failed, they were left orphaned.
--
-- This wraps the whole conversion in one SECURITY DEFINER function:
-- account (existing or new) -> contact -> optional opportunity -> mark
-- the lead converted + archived. Running as definer lets a CRM user
-- complete the archive step without being an admin, and a single
-- function call is one transaction, so a failure rolls everything back
-- (no orphans). Caller is still gated to CRM roles.
--
-- Field carry-over (the original sales request): the new account/contact/
-- opportunity inherit the lead's owner; the contact also gets the lead's
-- mql_date and an sql_date (the lead's SQL/SAL date if it had one, else
-- the conversion date, since converting is itself the SQL event).
-- ---------------------------------------------------------------------
...
begin
  ...
  -- Account: existing (left untouched) or new (inherits the lead owner).
  ...
  -- Contact: inherits owner + MQL/SQL from the lead.
  insert into public.contacts (
    ...
    true, p_lead_source, p_lead_id, v_lead.owner_user_id, v_mql, v_sql
  )
  returning id into v_contact_id;

  -- Opportunity (optional): inherits the lead owner.
  ...
  -- Mark the lead converted + archived. Definer rights let any CRM role
  -- complete this, not just admins.
  update public.leads
