-- Renewals: stop auto-creating "New signature needed" tasks + archive the
-- machine-created backlog (Nathan, 2026-07-21 morning, after Molly flagged
-- tasks for renewals she has no association with).
--
-- The generator (20260711210000, re-emitted 20260715230000) created a
-- signature task per non-auto-renew renewal, assigned to the PARENT deal's
-- owner. That had never fired at volume until the 2026-07-20 catch-up runs
-- created ~100 renewals -> 71 tasks, 36 already past-due, flooding task
-- lists, reminder bells, and the daily digest. Decision: renewal opps keep
-- being created, but NO automatic tasks (matches pre-generator behavior;
-- humans create their own tasks). Renewal-opp OWNERSHIP (assessor-first
-- inheritance) is a separate pending decision — see DOCKET.
--
--  1. Re-emit generate_upcoming_renewals_unsafe VERBATIM from
--     20260715230000 minus the task-insert block (and its v_task_due
--     declaration). Nothing else changes; the gated wrapper is untouched.
--  2. ARCHIVE (not delete — reversible) still-open machine-created
--     signature tasks: matched by the "Created by renewal automation."
--     body marker, completed ones untouched. Task lists, task-reminders,
--     and task-digest all filter archived_at, so the noise stops with it.
--
-- Idempotent: create-or-replace + archived_at-guarded update.
  ...
  begin
    ...
    loop
      begin
        ...
        -- (Signature-task auto-creation removed 2026-07-21 — renewals are
        -- created silently; people make their own tasks. See file header.)
...
-- 2. Archive the open machine-created signature tasks.
