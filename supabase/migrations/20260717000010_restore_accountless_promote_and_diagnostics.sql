-- ---------------------------------------------------------------------
-- Corrects 20260717000009 (which was built on a false premise and is
-- being reverted with Nathan's knowledge):
--
--   1. RESTORE the 2026-06-16 decision (20260616000004): company-less
--      leads promote to ACCOUNT-LESS contacts. No fabricated
--      "First Last (import)" accounts. Placeholder org names
--      (unknown / n-a / none / tbd / self / retired) also route to
--      account-less rather than matching or creating a junk account
--      (this part of 000009 is kept, per Nathan's report that 'Unknown'
--      placeholder orgs shouldn't become accounts).
--   2. ADD per-row failure diagnostics: error_detail [{lead_id, error,
--      code}] and ambiguous_detail [{lead_id, company}] (capped at 25
--      each) in the result JSON. The 2026-07-17 incident (205 promote
--      failures reported as a bare count with no ids and a single
--      unlabeled message) is undiagnosable with count-only reporting;
--      the June min(uuid) incident (see 20260616000005 header) was the
--      same failure shape. Never again.
--
-- Everything else is verbatim from 20260717000009 (itself verbatim from
-- 20260717000006 apart from the reverted fallback).
-- ---------------------------------------------------------------------
...
-- tag_usage_counts: count live contacts only, and carry the repo-standard
-- role guard (flagged in the 2026-07-17 audit: archived contacts inflated
-- counts; SECURITY DEFINER without a guard would outlive any future RLS
-- tightening on tags).
...
as $$
  select t.id, count(c.id)
    ...
    left join public.contacts c on c.id = ct.contact_id and c.archived_at is null
