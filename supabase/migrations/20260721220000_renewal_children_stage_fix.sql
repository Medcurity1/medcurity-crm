-- Renewal children: stage fix + one-time re-stage of ghost-stage rows.
--
-- The renewal generator has inserted children with stage 'proposal' since
-- 20260519000008 — a LEGACY enum value that 20260422000001 retired when the
-- pipeline moved to the SF-matching stage set. Legacy-stage rows are invisible
-- to the Opportunities list filters and the Pipeline board (both enumerate
-- current stages only) but DO appear in any not-closed filter (dashboard
-- "My Open Opportunities"), so auto-renewals surfaced on dashboards yet
-- vanished from every list view (reported by Margaret, 2026-07-21).
--
--  1. generate_upcoming_renewals_unsafe re-emitted VERBATIM from
--     20260721200000 (the restored-signature-tasks version — standup outcome
--     preserved: ownership rule, task block, suppression checks all intact)
--     with EXACTLY ONE change: children are created in 'proposal_conversation'.
--  2. One-time re-stage of any rows still on legacy stages, following the
--     20260422000001 mapping (proposal/verbal_commit -> proposal_conversation,
--     lead/qualified -> details_analysis). trg_opportunities_stage_history
--     records each transition; probability is identical for the mapped pairs
--     (both 'proposal' and 'proposal_conversation' default to 90); the
--     automations trigger matches zero configured rules (verified empty on
--     prod, 2026-07-21).
--
...
-- 2. One-time re-stage of legacy ghost-stage rows (April mapping).
