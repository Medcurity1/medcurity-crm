-- ---------------------------------------------------------------------
-- Make convert_lead idempotent (bug-hunt lead-convert A1).
--
-- Previously convert_lead never checked whether the lead was already
-- converted. A double-submit / stale dialog / direct re-call would create
-- a SECOND account + contact and overwrite the lead's converted_* pointers,
-- orphaning the first records. The UI hides converted leads but the RPC is
-- the real integrity boundary.
--
-- Fix: after loading the lead, if it's already converted, return the
-- EXISTING account/contact/opportunity pointers idempotently instead of
-- creating new records. Everything else is unchanged from 000007 (the
-- valid-enum lead_source guard is kept).
-- ---------------------------------------------------------------------
...
begin
  ...
  -- Load the lead first so we can short-circuit if it's already converted.
  ...
  -- IDEMPOTENT: if already converted, return the existing pointers rather
  -- than creating duplicate account/contact records.
  ...
  -- Scope the conversion write to a still-unconverted lead so a race loses
  -- safely (the winner's pointers stand; a loser updates 0 rows).
  update public.leads
