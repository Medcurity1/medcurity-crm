-- ---------------------------------------------------------------------
-- Renewal automation "fresh start" baseline (Nathan, 2026-07-11).
--
-- Context: renewal_automation_daily was never actually scheduled (fixed
-- in 20260711200000). Prod therefore has a backlog of ~101 contracts
-- already inside the renewal window that the automation would create all
-- at once on its first run. Nathan does not want the backlog dumped on
-- the team — the automation should START FRESH: contracts already inside
-- the window when it goes live stay manual; only contracts whose window
-- OPENS after go-live are automated.
--
-- Mechanics: renewal_automation_config.baseline_date is set to the date
-- this migration applies on each environment (staging: 2026-07-11; prod:
-- whenever it's promoted). A candidate is skipped when its anniversary
-- is on or before baseline_date + lookahead_days — i.e. its renewal
-- window had already opened at go-live. Anniversaries that cross into
-- the window after baseline behave exactly as before (including the
-- lookback catch-up for missed runs). NULL baseline = old behavior.
--
-- Touches the two *_unsafe bodies (renamed by 20260701000004; the
-- admin-gated public wrappers pass through and are unchanged). Both are
-- re-emitted VERBATIM from 20260625000009 with only the baseline change;
-- return shapes are unchanged so the wrappers keep working.
-- ---------------------------------------------------------------------

begin;

-- ---------------------------------------------------------------------
-- 1. Config column + per-env seed
-- ---------------------------------------------------------------------
alter table public.renewal_automation_config
  add column if not exists baseline_date date;

comment on column public.renewal_automation_config.baseline_date is
  'Fresh-start date: contracts whose anniversary is on/before baseline_date + lookahead_days were already inside the renewal window when the automation went live on this environment and are never auto-created (manual backlog). Set automatically by migration 20260711210000; NULL disables the filter.';

update public.renewal_automation_config
  set baseline_date = coalesce(baseline_date, current_date)
  where id = 1;

-- ---------------------------------------------------------------------
-- 2. generate_upcoming_renewals_unsafe — re-emitted from 20260625000009
--    with ONLY the baseline gate added to the candidate WHERE.
-- ---------------------------------------------------------------------
...
as $$
  ...
  order by
    case
      ...
      when c.account_status <> 'active'                                   then 8
