-- Nexus Your Day: requests branch (docket C2 round-2 decisions).
--
-- The briefing must serve non-sales users as well as Home did: for the
-- teammates who ACT on requests (request_routing decides who that is per
-- type), open requests are day-one work and belong in the queue. Adds a
-- sixth branch to rep_day_queue: pending requests routed to the caller,
-- ranked between hot replies and plain due tasks, bumped by priority.
-- Row shape is unchanged (the request id rides in item_key); everything
-- else is byte-identical to 20260729150000.
--
-- RLS note (SECURITY INVOKER): requests_select shows a row to its
-- requester or to admins. Every routed actor today is an admin, so the
-- branch works; if a non-admin is ever routed, the select policy needs a
-- routing arm before their briefing can show these.
...
comment on function public.rep_day_queue(int) is
  'Nexus "Your Day" ranked next-best-action queue (docket C2). SECURITY INVOKER + explicit owner filters on every branch. Branches: replies, paused outreach, tasks, routed pending requests, renewal windows, stale deals. Pacific-time day boundaries; dash-free UI copy; snoozes come from day_queue_snoozes.';
