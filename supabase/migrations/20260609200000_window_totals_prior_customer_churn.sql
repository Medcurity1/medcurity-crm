-- ---------------------------------------------------------------------
-- Add prior_churn_pct_customers to f_financial_saas_metrics_window_totals.
--
-- The headline churn metric is now CUSTOMER (client-count) churn, not
-- revenue churn. The window-totals function already returns
-- churn_pct_customers for the selected period, but the KPI card's
-- "vs prior period" delta needs the prior period's customer churn too.
-- Adding a return column requires DROP + CREATE (CREATE OR REPLACE
-- cannot change a function's return shape). The app reads columns by
-- name, so adding one is safe.
--
-- Churn definition unchanged: lost / (active base + lost), bounded 0-100%.
-- ---------------------------------------------------------------------

begin;

drop function if exists public.f_financial_saas_metrics_window_totals(date, date);

create function public.f_financial_saas_metrics_window_totals(
...
as $$
  ...
  select
    ...
    case when pb.pr_start is not null
         then coalesce(p.p_lost_count::numeric / nullif(p.p_customer_count + p.p_lost_count, 0), 0)
