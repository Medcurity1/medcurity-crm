-- ---------------------------------------------------------------------
-- Opportunity delete for reps + renewals that stay deleted.
--
-- Jordan S (2026-06-12) asked for a way to delete opportunities --
-- mainly invalid auto-generated renewals. Three parts:
--
-- 1. RLS: sales/renewals may DELETE opportunities, EXCEPT closed_won
--    (revenue history stays admin-only). Admins unchanged.
-- 2. renewal_suppressions: deleting a generated renewal previously
--    caused the daily generator to recreate it (its dedup found no
--    live child). A BEFORE DELETE trigger records the parent so the
--    generator knows the renewal was deliberately removed. Archived
--    children also count as already-generated now.
-- 3. generate_upcoming_renewals + preview_upcoming_renewals re-emitted
--    verbatim from 20260520000000 with ONLY the dedup changed, so the
--    admin preview matches what the generator will actually do.
-- ---------------------------------------------------------------------

begin;

create table if not exists public.renewal_suppressions (
...
alter table public.renewal_suppressions enable row level security;

drop policy if exists "renewal_suppressions_select" on public.renewal_suppressions;
create policy "renewal_suppressions_select" on public.renewal_suppressions
  for select to authenticated using (true);

-- Admins can undo a suppression (deleted the wrong renewal and want
-- the generator to recreate it).
drop policy if exists "renewal_suppressions_admin_delete" on public.renewal_suppressions;
create policy "renewal_suppressions_admin_delete" on public.renewal_suppressions
  for delete to authenticated using (public.is_admin());

create or replace function public.record_renewal_suppression()
returns trigger
language plpgsql
security definer
...
as $fn$
begin
  if old.renewal_from_opportunity_id is not null then
    insert into public.renewal_suppressions
      (source_opportunity_id, deleted_child_name, deleted_by)
    values (old.renewal_from_opportunity_id, old.name, auth.uid())
...
drop trigger if exists trg_opportunities_renewal_suppression on public.opportunities;
create trigger trg_opportunities_renewal_suppression
before delete on public.opportunities
for each row execute function public.record_renewal_suppression();

-- Reps may delete non-closed-won opportunities; closed-won stays admin.
...
create policy "opportunities_delete" on public.opportunities
  ...
  using (
    public.is_admin()
    or (
      public.current_app_role() in ('sales', 'renewals')
      and stage <> 'closed_won'
    )
  );

-- Generator + preview, re-emitted with the new dedup ------------------
