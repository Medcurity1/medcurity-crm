-- ============================================================
-- Activities: created_by_automation flag (Summer's "Automation badge" ask).
--
-- Machine-created tasks are stamped owner = the account/opp owner, and every
-- feed renders just the owner's name — so renewal signature tasks read as if
-- the rep typed them (Summer, 7/22 follow-up; bit again during the 7/27
-- duplicate-renewal investigation where Margaret worked machine tasks as
-- normal work). The activities table had NO marker: the body sentence
-- "Created by renewal automation." was the only tell.
--
--  1. `created_by_automation boolean not null default false` on activities —
--     same name/semantics as the opportunities column.
--  2. Backfill: renewal signature tasks (the standard body sentence) and
--     campaign-spawned rows (anything carrying campaign_enrollment_id).
--     The frontend badge checks flag OR campaign_enrollment_id, so future
--     campaign spawns from the edge fn stay badged even before the edge fn
--     learns the new column.
--  3. generate_upcoming_renewals_unsafe re-emitted VERBATIM from
--     20260727130000 (covering-dedup version) with EXACTLY ONE change: its
--     signature-task insert stamps created_by_automation = true.
--
-- Idempotent: add-if-not-exists + guarded backfill + create-or-replace.
-- ============================================================
...
comment on column public.activities.created_by_automation is
  'True for rows a machine wrote (renewal signature tasks, campaign-spawned tasks/logs). Feeds show an "Automation" badge so they never read as human-written.';

update public.activities
   set created_by_automation = true
 where created_by_automation = false
   and (
     (activity_type = 'task' and body like '%Created by renewal automation.%')
     or campaign_enrollment_id is not null
   );

-- ---------- Generator re-emit (20260727130000 + flag on the task insert) ----------
  ...
  begin
    ...
    loop
      begin
        ...
        -- Covering-deal dedup (2026-07-27): a renewal made by hand or
