...
describe("known Campaigns build gaps", () => {
  ...
  it("offers friendly first-name and organization controls for subjects in every builder", () => {
    ...
    const list = read("src/features/playbook/SequenceStepList.tsx");
    const wizard = read("src/features/playbook/CampaignWizard.tsx");

    expect(editor).toMatch(/SequenceStepList/);
    expect(list.match(/Add to subject/g)).toHaveLength(1);
    expect(wizard.match(/Add to subject/g)).toHaveLength(1);
    expect(list).toMatch(/subject_template: insertAuthorToken[\s\S]*AUTHOR_TOKENS\.firstName/);
    expect(list).toMatch(/subject_template: insertAuthorToken[\s\S]*AUTHOR_TOKENS\.organization/);
    expect(wizard).toMatch(/subject: insertAuthorToken[\s\S]*AUTHOR_TOKENS\.firstName/);
    expect(wizard).toMatch(/subject: insertAuthorToken[\s\S]*AUTHOR_TOKENS\.organization/);
  });
  ...
  it("keeps the chosen build method and campaign title when the unified builder autosaves or switches methods", () => {
    ...
    expect(wizard).toMatch(/setEditingSequence\(true\)/);
